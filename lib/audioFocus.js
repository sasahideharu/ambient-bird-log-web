...
export function focusGainDb(f, { fLo, fHi, topHz }, settings = DEFAULT_FOCUS) {
  const { mode, strengthDb, curve, octaveDb } = { ...DEFAULT_FOCUS, ...settings };
  if (f >= fLo && f <= fHi) return 0;

  if (mode === "octave") {
    const octaves = f < fLo ? (f <= 0 ? Infinity : Math.log2(fLo / f)) : Math.log2(f / fHi);
    return -Math.min(strengthDb, octaveDb * octaves);
  }
  ...
  return -strengthDb * Math.pow(r, curve);
}
...
// samples：元の波形（Float32Array・モノラル）／sampleRate：サンプリング周波数
...
// 戻り値：加工後の波形（Float32Array・切り出した長さ）
export function renderFocused(samples, sampleRate, range, settings = DEFAULT_FOCUS, options = {}) {
  ...
  const i1 = Math.min(samples.length, Math.ceil(range.t1 * sampleRate));
  const len = i1 - i0;
  ...
  padded.set(samples.subarray(i0, i1), FFT_SIZE);
