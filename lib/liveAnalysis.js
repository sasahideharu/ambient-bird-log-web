...
const YIELD_EVERY_BLOCKS = 12; // 約0.3秒ぶん変換するごとに、画面に処理を譲る（スペクトログラムの動きを止めないため）
...
// 録音の画面を開いたときに呼ぶ。解析サーバーを、先に起こしておく（起きるまで待つ。約20秒）。戻り値：起きたら true
export async function warmLiveServer() {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), WARM_TIMEOUT_MS);
  try {
    const res = await fetch(`${ANALYZER_URL}/live/warm`, { method: "POST", headers: await authHeaders(), signal: ctrl.signal });
    return res.ok;
  } catch (err) {
    console.error(err);
    return false;
...
// Float32 の音 → MP3（Uint8Array）。長いあいだ画面を固めないよう、途中で、休みを入れる
async function encodeMp3Bytes(int16, sampleRate) {
  const encoder = new Mp3Encoder(1, sampleRate, KBPS);
  const parts = [];
  let size = 0;
  for (let i = 0, block = 0; i < int16.length; i += BLOCK, block++) {
...
export function createLiveAnalysis({ getLocation, isEnabled = () => true, onResult, onState }) {
  ...
  const stats = { status: "idle", message: "", sent: 0, ok: 0, failed: 0, skipped: 0, latencySec: null };
  ...
  async function send(endSample) {
    ...
    try {
      ...
      firstRequest = false;
      stats.ok += 1;
      emit({ status: latencySec > 6 ? "slow" : "ok", message: "", latencySec: Math.round(latencySec * 10) / 10 });
      if (!stopped) {
        onResult?.({
