...
// 音の生のバイト列を、まとめて読む（あとで解析、など）。kind："pcm"（無圧縮）| "aac"（別の録音）。無ければ null
export async function readBytes(id, kind) {
  if (!isNative()) {
  ...
  const { Filesystem, Directory } = await fs();
  try {
    const { data } = await Filesystem.readFile({ path: `${dirOf(id)}/${FILES[kind]}`, directory: Directory.Data });
