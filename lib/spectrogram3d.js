...
//    ・再生位置：再生中だけ、横軸の中心の縦線と、その瞬間の左右の音の輪郭（白い線）が、奥行き（時間）に沿って進む（再生していないときは、出さない）
...
export function createViewer({ canvas, onFrame, onUserRotate, interactive = true }) {
  ...
  function buildAxes() {
    ...
    for (let s = 0; s <= data.duration + 1e-6; s += step) {
      ...
      const tl = label(`${s % 1 === 0 ? s : s.toFixed(1)}s`, { size: 0.11 });
      tl.position.set(tx + 0.28, 0.0, z);
      axesGroup.add(tl);
    }

    if (S.grid) axesGroup.add(buildGrid(W, D, hzStep, step));
    buildPlayhead(axesGroup);
    world.add(axesGroup);
    dirty = true;
  }

  // ---------- 再生位置（中心の縦線＋その瞬間の左右の音の輪郭＋音のある部分の光る帯） ----------
  //   中心の縦線（横軸の中心・下から上まで）＝いま、どの時刻か。四角い枠は描かない。
  //   輪郭線と光る帯＝その瞬間に、音が出ている周波数（音の無い所は、光らない）
  function buildPlayhead(parent) {
    const { B } = dims;
    ...
    // 中心の縦線：もともとの四角い枠と同じ、細さ（1本）と色（白・不透明度 0.5）。主張しすぎないように
    const center = new THREE.Line(
      new THREE.BufferGeometry().setFromPoints([new THREE.Vector3(0, 0, 0), new THREE.Vector3(0, H, 0)]),
    ...
    for (let side = 0; side < 2; side++) {
      ...
      for (const [geom, kind, copies, order] of [[g, "quiet", 3, 8], [gLoud, "loud", 5, 9]]) { // 写しの数（layoutSliceCopies の patterns と合わせる）
