...
export function createViewer({ canvas, onFrame, onUserRotate }) {
      ...
      for (const t of dims.lineFrames) {
        ...
        for (let b = 0; b < B - 1; b++) {
          for (const bb of [b, b + 1]) {
            ...
            col[c++] = vis ? 1 : 0.1; // 音の無い所（中心の線）は、ほとんど見えなくする
          }
        }
      }
  ...
  function buildAxes() {
    ...
    axesGroup.add(line([[0, 0, -D / 2], [0, 0, D / 2]], 0x5d6979, 0.9));
    axesGroup.add(line([[0, 0, D / 2], [0, H, D / 2]], 0x3a4350, 0.6));
  ...
  function buildPlayhead(parent, Wax) {
    ...
    const mkSlice = () => {
      ...
      const l = new THREE.Line(g, new THREE.LineBasicMaterial({ color: 0xffffff, transparent: true, opacity: 0.95, depthTest: false }));
      l.renderOrder = 7;
      l.frustumCulled = false;
      return l;
    };
