...
//    録音が終わったとき、①が、ほぼ全部（99%以上）取れていれば、①を「正式な録音（master）」にする。足りなければ、②を正式にする。
...
const PCM_MIN_COVERAGE = 0.99; // 無圧縮が、これ以上取れていれば、無圧縮を正式な録音にする
...
export async function startRecording({ recorder, location, onEnded }) {
  ...
  const snapshot = () => {
    ...
    meta.audio.pcm.coverage = wallSec > 0 ? Math.min(1, pcmSamples / cap.sampleRate / wallSec) : null;
  ...
  return {
    ...
    status: () => {
      ...
      return {
        ...
        pcmCoverage: wallSec > 1 ? Math.min(1, pcmSamples / cap.sampleRate / wallSec) : 1,
