...
//   onDiscarded(reason)：保存せずに「破棄」したときに、1回だけ呼ばれる（onEnded は呼ばれない）
//   onChunk(chunk, info)：音のかたまりが届くごとに呼ばれる（録音の始めから。info.sampleRate つき。リアルタイム解析が使う）
//   戻り値：操作の道具（controller）
export async function startRecording({ recorder, location, onEnded, onDiscarded, onChunk: onChunkArg }) {
  ...
  const meta = {
    ...
    live: null, // リアルタイム解析の結果（参考）：{ version, timeBase, stepSec, windowSec, windows: [{ t, b: [[学名, 信頼度]], p: [[学名, logit]] }] }
  ...
  if (onChunkArg) subscribers.add(onChunkArg); // 録音の始めの音から、受け取れるように、先に登録する
  let capStartMs = 0;
    ...
    cap = await startPcmCapture({
      ...
      onChunk: (chunk, info) => {
        ...
        if (subscribers.size > 0) {
          const ci = { ...info, sampleRate: cap?.sampleRate };
          subscribers.forEach((fn) => fn(chunk, ci));
        }
      },
    });
  } catch (err) {
  ...
  // 録音を止めて、マイク・書き込みを片づける（保存する・破棄する、共通の下ごしらえ）
  ...
  // 保存せずに、やめる（間違えて始めたときなど）。ここまで端末に書いた分ごと、消す。戻したら、二度と戻せない
  async function discard(reason = "破棄ボタン") {
    if (stopped) return stopped; // すでに、保存の途中・保存ずみ（あとから、一覧の「削除」で消せる）
    stopped = (async () => {
      await teardown();
      await store.deleteRecording(id).catch((err) => console.error(err));
      onDiscarded?.(reason);
      return null;
    })();
    return stopped;
  ...
  return {
    ...
    // リアルタイム解析の結果を、記録に足す（あとで、参考にする。1500回＝75分まで）
    addLive: (entry) => {
      if (!meta.live) meta.live = { version: 1, timeBase: "pcm", stepSec: 3, windowSec: 5, windows: [] };
      if (meta.live.windows.length < 1500) meta.live.windows.push(entry);
    },
    onChunk: (fn) => {
      subscribers.add(fn);
