...
//    ・公開したものは、そのままでは、直せない・消せない（データベース側で制限）。「取り下げ」（unpublishEdit）で、下書きに戻してから、直す・消す
...
//   公開　　：解析の記録を登録（detections）→ published_at を入れる。それ以降は、取り下げる（unpublishEdit）まで、直せない・消せない
...
// 公開した印を付ける（記録の登録が終わったあとに呼ぶ。付けたら、取り下げるまで、直せない・消せない）
...
// 公開を取り下げる（管理者本人の、公開済みの編集だけ）。書き出した録音の記録（detections）と、Perch の意見を消して、公開日時を空にする
//   → 下書きに戻って、直す・消す・もう一度公開する、ことができる。確認（確定・修正）と、保管場所の録音ファイルは、残る。
//   データベースの命令 unpublish_audio_edit（db/migrations/2026-09-21_unpublish_audio_edit.sql）が、まとめて行う。戻り値：{ name, detections_deleted, opinions_deleted }
export async function unpublishEdit(dbId) {
  const { data, error } = await supabase.rpc("unpublish_audio_edit", { p_id: dbId });
  if (error) throw error;
  return data;
}
