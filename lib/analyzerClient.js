...
// 戻り値：{ results: { 名前: { rows, week, error? } }, warnings, meta, elapsedSec }
export async function analyzeMp3Files({ names, minConf, location, useWeek, stereo = "best", onProgress }) {
  ...
  for (let i = 0; i < names.length; i += ANALYZER_MAX_FILES) {
    ...
    try {
      res = await fetch(`${ANALYZER_URL}/analyze`, {
        ...
        body: JSON.stringify({ files: chunk, min_conf: minConf, location: location ?? null, use_week: useWeek, stereo }),
      });
    } catch (err) {
      console.error(err);
