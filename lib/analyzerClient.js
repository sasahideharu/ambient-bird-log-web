...
export async function analyzeMp3Files({ names, minConf, location, useWeek, onProgress }) {
  ...
  for (let i = 0; i < names.length; i += ANALYZER_MAX_FILES) {
    ...
    try {
      res = await fetch(`${ANALYZER_URL}/analyze`, {
        ...
        body: JSON.stringify({ files: chunk, min_conf: minConf, location: location ?? null, use_week: useWeek }),
      });
    } catch (err) {
      console.error(err);
