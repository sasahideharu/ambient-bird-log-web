...
function Row({ e, model }) {
  ...
  return (
    <li
      className={`rounded-lg px-1.5 py-1 leading-tight ${e.agreeNow ? "bg-[#3E9B5F]/30 ring-1 ring-[#8FE0B0]" : e.agreed ? "bg-white/15" : e.recent ? "bg-white/10" : ""}`}
    >
      <div className={`break-words text-[11px] ${e.agreed ? "font-bold" : ""} ${e.recent ? "text-white" : "text-white/55"}`}>
        {e.agreed && <span className="text-[#8FE0B0]">✓</span>}
        {e.common ?? e.sci}
      </div>
      <div className="flex items-baseline justify-between gap-1 text-[9px] tabular-nums text-white/55">
        <span className={`text-[10px] ${strong || model === "bn" ? "font-bold text-white" : ""}`}>{score}</span>
        <span>
...
function Column({ title, rows, model, empty }) {
  return (
    <div className="min-w-0">
      <div className="mb-1 text-center text-[11px] font-bold text-white/80">{title}</div>
      <ul className="max-h-52 space-y-1 overflow-y-auto p-0.5">
        {rows.length === 0 ? <li className="px-1 py-2 text-center text-[10px] text-white/45">{empty}</li> : rows.map((e) => <Row key={e.sci} e={e} model={model} />)}
      </ul>
    </div>
  );
}
...
export default function LiveBirds({ list, state, enabled, finished = false }) {
  ...
  return (
    <div className="mt-3">
      <div className={`mb-1.5 flex items-center justify-between text-[10px] ${warn ? "text-[#F0C27B]" : "text-white/55"}`}>
      ...
      {state?.message && enabled && !finished && <p className="mb-1.5 text-[10px] leading-relaxed text-[#F0C27B]">{state.message}</p>}
      {list.agreed.length > 0 && (
        <div className="mb-2 rounded-lg bg-[#3E9B5F]/25 px-2 py-1.5 text-[11px] leading-relaxed text-[#DDF3E4]">
